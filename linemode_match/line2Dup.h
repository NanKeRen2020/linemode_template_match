...
struct Template
{
    ...
    {
        return features.size() < rhs.features.size();
    }
...
public:
    ...
    std::vector<Match> match(cv::Mat source, float threshold,
                                                     const std::vector<std::string> &class_ids = std::vector<std::string>(),
                                                     const cv::Mat mask = cv::Mat());

    std::vector<Match> match(cv::Mat source, float threshold, const std::map<std::string, std::vector<std::vector<Template>>>& tmmpates_maps,
                             const std::vector<std::string> &class_ids = std::vector<std::string>(),  const cv::Mat mask = cv::Mat()); 
                                                  
                                                     

    std::vector<Match> match_fast(cv::Mat source, float threshold, const std::vector<std::string> &class_ids = std::vector<std::string>(),
                                  int down_layer = 1, int search_number = 1, int search_expand = 20, const cv::Mat mask = cv::Mat());

    bool removeTemplate(const std::vector<std::string>& class_ids);
    ...
    std::vector<std::string> isLoaded() const;

    bool isLoaded(const std::string& class_id) const;
...
protected:
    ...
    TemplatesMap rough_class_templates;
    TemplatesMap original_class_templates;
...
public:
    ...
    void set_src(const cv::Mat& src, const cv::Mat& mask = cv::Mat())
    {
        this->src = src;
        if(mask.empty()){
            // make sure we have masks
    ...
    int produce_infos(){
        ...
        */
        ...
        {
            float scale = scale_range[0];
            for(int i = 0; i < angle_range.size(); ++i){
                    infos.emplace_back(angle_range[i], scale);
            }
        ...
        {
            float angle = angle_range[0];
            scale_num = scale_range.size();
            for(int i = 0; i < scale_range.size(); ++i)
            {
                 infos.emplace_back(angle, scale_range[i]);
            }
        }
        else if (angle_range.size() == 2 && scale_range.size() > 2)
        {
        ...
        else if (angle_range.size() > 2 && scale_range.size() > 2)
        {
            scale_num = scale_range.size();
            for(int i = 0; i < scale_range.size(); ++i){
                for(int j = 0; j < angle_range.size(); ++j){
                    infos.emplace_back(angle_range[j], scale_range[i]);
                }
            }
        }
        else
